...
//                     拷贝构造函数调用时机 1 使用一个已经创建完毕的对象来初始化一个对象 2值传递的方式给函数参数传值  3 值方式返回局部对象  
//                构造函数的调用规则(当创建一个类时，编译器默认添加三个函数，构造函数，析构函数，拷贝构造--值拷贝) 如果我们写了有参构造函数，编译器不会提供默认构造函数，但还会提供拷贝函数构造  如果我们自己写了拷贝函数则编译器不会提供任何其他的函数
//                      